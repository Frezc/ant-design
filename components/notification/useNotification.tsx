...
// replace newNotificationInstance in rc-notification/src/Notification.jsx
function newNotificationInstance(properties: any, callback: any) {
...
function useNotification() {
  ...
  const notificationInstanceRef = React.useRef<{ [key: string]: any }>({});
  function getNotificationInstance(
    {
      prefixCls,
      placement = defaultPlacement,
    ...
    const cacheKey = `${prefixCls}-${placement}`;
    if (notificationInstanceRef.current[cacheKey]) {
      callback(notificationInstanceRef.current[cacheKey]);
      return;
    }
    const el = newNotificationInstance(
      {
        prefixCls,
        className: `${prefixCls}-${placement}`,
        style: getPlacementStyle(placement, top, bottom),
      ...
      (notification: any) => {
        notificationInstanceRef.current[cacheKey] = notification;
        callback(notification);
      },
    );
    setElements(elements.concat(React.cloneElement(el, { key: cacheKey })));
  }
