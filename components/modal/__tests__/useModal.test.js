import Modal from '..';
import * as React from 'react';
import { renderHook } from '@testing-library/react-hooks';
import { mount } from 'enzyme';
import { act } from 'react-test-renderer';

const Wrapper = ({ elements }) => <div>{elements}</div>;

describe('Modal.useModal hook works correctly', () => {
  const errorSpy = jest.spyOn(console, 'error').mockImplementation((...m) => {
    console.warn('error', ...m);
  });
  let hook;
  let wrapper = mount(<Wrapper />);

  beforeEach(() => {
    hook = renderHook(() => Modal.useModal());
    wrapper.mount();
  });

  afterEach(() => {
    wrapper.unmount();
    errorSpy.mockReset();
  });

  afterAll(() => {
    errorSpy.mockRestore();
  });

  function $$(className) {
    return document.body.querySelectorAll(className);
  }

  function rerender() {
    wrapper.setProps({ elements: hook.result.current[1] });
  }

  function open(args) {
    act(() => {
      hook.result.current[0].confirm({
        title: 'Want to delete these items?',
        content: 'some descriptions',
        ...args,
      });
    });
    rerender();
  }

  function trigger(cb) {
    act(() => {
      cb();
    });
    rerender();
  }

  it('basic hook', () => {
    const { result } = renderHook(() => Modal.useModal());
    act(() => {
      result.current[0].confirm({ title: 'hello', content: 'world' });
    });

    expect(result.current[1].length).toBe(1);
  });

  it('trigger onCancel once when click on cancel button', () => {
    const onCancel = jest.fn();
    const onOk = jest.fn();
    open({
      onCancel,
      onOk,
    });
    // first Modal
    trigger(() => {
      $$('.ant-btn')[0].click();
    });
    expect(onCancel.mock.calls.length).toBe(1);
    expect(onOk.mock.calls.length).toBe(0);
  });

  it('trigger onOk once when click on ok button', () => {
    const onCancel = jest.fn();
    const onOk = jest.fn();
    open({
      onCancel,
      onOk,
    });
    // second Modal
    trigger(() => {
      $$('.ant-btn-primary')[0].click();
    });
    expect(onCancel.mock.calls.length).toBe(0);
    expect(onOk.mock.calls.length).toBe(1);
  });

  it('should allow Modal.comfirm without onCancel been set', () => {
    open();
    // Third Modal
    trigger(() => {
      $$('.ant-btn')[0].click();
    });
    expect(errorSpy).not.toHaveBeenCalled();
  });

  it('should allow Modal.comfirm without onOk been set', () => {
    open();
    // Fourth Modal
    trigger(() => {
      $$('.ant-btn-primary')[0].click();
    });
    expect(errorSpy).not.toHaveBeenCalled();
  });

  it('should emit error when onOk return Promise.reject', () => {
    const error = new Error('something wrong');
    open({
      onOk: () => Promise.reject(error),
    });
    // Fifth Modal
    trigger(() => {
      $$('.ant-btn-primary')[0].click();
    });
    // wait promise
    return Promise.resolve().then(() => {
      expect(errorSpy).toHaveBeenCalledWith(error);
    });
  });

  it('shows animation when close', () => {
    jest.useFakeTimers();
    open();
    trigger(() => {
      $$('.ant-btn')[0].click();
    });
    expect($$('.ant-modal-confirm')).toHaveLength(1);
    trigger(() => {
      jest.runAllTimers();
    });
    expect($$('.ant-modal-confirm')).toHaveLength(0);
    jest.useRealTimers();
  });

  it('ok only', () => {
    open({ okCancel: false });
    expect($$('.ant-btn')).toHaveLength(1);
    expect($$('.ant-btn')[0].innerHTML).toContain('OK');
  });

  it('allows extra props on buttons', () => {
    open({ okButtonProps: { disabled: true }, cancelButtonProps: { 'data-test': 'baz' } });
    expect($$('.ant-btn')).toHaveLength(2);
    expect($$('.ant-btn')[0].attributes['data-test'].value).toBe('baz');
    expect($$('.ant-btn')[1].disabled).toBe(true);
  });

  it('should close modals when click confirm button', () => {
    jest.useFakeTimers();
    ['info', 'success', 'warning', 'error'].forEach(type => {
      trigger(() => {
        hook.result.current[0][type]({
          title: 'title',
          content: 'content',
        });
      });
      expect($$(`.ant-modal-confirm-${type}`)).toHaveLength(1);
      trigger(() => {
        $$('.ant-btn')[0].click();
      });
      trigger(() => {
        jest.runAllTimers();
      });
      expect($$(`.ant-modal-confirm-${type}`)).toHaveLength(0);
    });
    jest.useRealTimers();
  });

  it('could be update', () => {
    jest.useFakeTimers();
    ['info', 'success', 'warning', 'error'].forEach(type => {
      let instance;
      trigger(() => {
        instance = hook.result.current[0][type]({
          title: 'title',
          content: 'content',
        });
      });
      expect($$(`.ant-modal-confirm-${type}`)).toHaveLength(1);
      expect($$('.ant-modal-confirm-title')[0].innerHTML).toBe('title');
      expect($$('.ant-modal-confirm-content')[0].innerHTML).toBe('content');
      trigger(() => {
        instance.update({
          title: 'new title',
          content: 'new content',
        });
      });
      expect($$(`.ant-modal-confirm-${type}`)).toHaveLength(1);
      expect($$('.ant-modal-confirm-title')[0].innerHTML).toBe('new title');
      expect($$('.ant-modal-confirm-content')[0].innerHTML).toBe('new content');
      trigger(() => {
        instance.destroy();
      });
      trigger(() => {
        jest.runAllTimers();
      });
    });
    jest.useRealTimers();
  });

  it('could be destroy', () => {
    jest.useFakeTimers();
    ['info', 'success', 'warning', 'error'].forEach(type => {
      let instance;
      trigger(() => {
        instance = hook.result.current[0][type]({
          title: 'title',
          content: 'content',
        });
      });
      expect($$(`.ant-modal-confirm-${type}`)).toHaveLength(1);
      trigger(() => {
        instance.destroy();
      });
      trigger(() => {
        jest.runAllTimers();
      });
      expect($$(`.ant-modal-confirm-${type}`)).toHaveLength(0);
    });
    jest.useRealTimers();
  });

  it('could be Modal.destroyAll', () => {
    jest.useFakeTimers();
    ['info', 'success', 'warning', 'error'].forEach(type => {
      trigger(() => {
        hook.result.current[0][type]({
          title: 'title',
          content: 'content',
        });
      });
      expect($$(`.ant-modal-confirm-${type}`)).toHaveLength(1);
    });
    trigger(() => {
      hook.result.current[0].destroyAll();
    });
    ['info', 'success', 'warning', 'error'].forEach(type => {
      trigger(() => {
        jest.runAllTimers();
      });
      expect($$(`.ant-modal-confirm-${type}`)).toHaveLength(0);
    });
    jest.useRealTimers();
  });

  it('prefixCls', () => {
    open({ prefixCls: 'custom-modal' });
    expect($$('.custom-modal-mask')).toHaveLength(1);
    expect($$('.custom-modal-wrap')).toHaveLength(1);
    expect($$('.custom-modal-confirm')).toHaveLength(1);
    expect($$('.custom-modal-confirm-body-wrapper')).toHaveLength(1);
  });

  it('should be Modal.confirm without mask', () => {
    open({ mask: false });
    expect($$('.ant-modal-mask')).toHaveLength(0);
  });
});
